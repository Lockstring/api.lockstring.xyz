if not skey then
    return game.Players.LocalPlayer:Kick("skey not found!")
end
do
    ...
    if not IsCClosure(HttpRequest) then return end
    ...
    local function hmac(key, msg)
        ...
        for i=1,64 do
            local b = byte(key,i)
            ipad[i] = char(bit.bxor(b,0x36))
            opad[i] = char(bit.bxor(b,0x5c))
        end
        local inner = sha256(concat(ipad)..msg)
        local final = sha256(concat(opad)..inner)
        return final:gsub(".", function(c) return format("%02x",byte(c)) end)
    end
    local hex    = hmac(base_key, message)
    local signed = hmac(base_key, skey.."|"..hex)
    local Response = HttpRequest({
        Url     = URL1,
        Method  = "POST",
        Headers = {["Content-Type"] = "application/json"},
        Body    = format('{"a":"%s","b":"%s","c":"%s"}', skey, hex, signed)
    })
    ...
    for _,entry in ipairs(data.responses) do
        if hmac(base_key, entry.a) == entry.b then
    ...
    if real_session then
        signed = hmac(base_key, real_session.."|"..hex)
    end
    Response = HttpRequest({
        Url     = URL2,
        Method  = "POST",
        Headers = {["Content-Type"] = "application/json"},
        Body    = format('{"key":"%s","sig":"%s"}', skey, signed)
    })
