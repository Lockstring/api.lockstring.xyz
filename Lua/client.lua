...
do
    ...
    local function sha256(msg)
        ...
        for i=1,len(msg),64 do
            ...
            for j=0,63 do
                ...
                h,g,f,e,d,c,b,a=
                    g,f,e,
                    bit.band(d+temp1,0xffffffff),
                    c,b,a,
                    bit.band(temp1+temp2,0xffffffff)
            end

            local vals={a,b,c,d,e,f,g,h}
            for j=1,8 do
                H[j]=bit.band(H[j]+vals[j],0xffffffff)
            end
        end
    ...
    local hex=final:gsub(".",function(c)
    ...
    for _, entry in ipairs(data.responses) do
        ...
        local inner = sha256(concat(ipad) .. session)
        local final = sha256(concat(opad) .. inner)

        local session_hmac = final:gsub(".", function(c)
            return format("%02x", byte(c))
        end)
